...
class blog extends \db{
    ...
    public function getLast($m=4) {
        $q="SELECT *,LEFT(`content`, 256)as `lcontent` FROM `blog` ORDER BY `pdate` DESC,`id` DESC LIMIT 0,$m";
        return $this->get_result($q);
    }
        public function getBlog($offset = 0, $page_result = 10) {
               
        $zapros = "SELECT SQL_CALC_FOUND_ROWS *, LEFT(`content`, 620) as `lcontent` FROM `blog` ORDER BY `pdate` DESC,`id` DESC limit $offset, $page_result;";
        //echo $zapros;
