...
<div class="modal-dialog modal-lg" style="overflow:initial">
	<div class="modal-content">
		...
		<h4 class="modal-title"><i class="fa fa-image"></i>&nbsp;&nbsp;Image manager</h4>
...
<div class="modal fade" id="imagemodaldelete" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm">
	...
	<div class="btn-warning modal-header">
      ...
      <p style="text-align:right;padding-right:20px">
        <button type="button" id="delete_image" class="btn btn-primary close-modal">Yes</button>&nbsp;
	<button type="button" class="btn btn-default close-modal">No</button>
      </p>
    </div>
  </div>
</div>
...
$('#imageBrouser').on('click', '.insert-image' ,function () {
			...
			$.ajax({
				...
				complete: function() {
					...
					$('#button-upload').prop('disabled', false);
                                        $.get('<?= WWW_ADMIN_PATH ?>fs/reload/' + '<?= $target; ?>')
                                .done(function (data) {
...
$('.insert-image').on('click',function(e){
                ...
                $.ajax({
                        type: 'post',
                        url: '<?=WWW_ADMIN_PATH?>fs/manager',
                        data: {target:target},
                        dataType: 'html',
                        success: function (html) {
                        //$('.note-editor').append('<div id="modal-image" class="modal" aria-hidden="false" tabindex="-1" role="dialog" aria-label="Add Image">' + html + '</div>');
                            $('#imageBrouser').html(html);
                            $('#imageBrouser').modal();
                          }
                       });
                      });
