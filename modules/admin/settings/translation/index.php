...
foreach ($tran->getLang($l->lang_eu) as $k=>$frases)
{
    ...
    $fr.="<form method='post' action='".WWW_ADMIN_PATH."settings/translation/save'>";
    $fr.=$f->input('lang', '', 'hidden', 'lang', '', $l->lang_eu);
    $fr.=$f->input('ident', '', 'hidden', 'ident', '', $frases->ident);
    $fr.=$f->input('sa'.$k, $frases->ident, 'text', 'langtext','введите перевод',htmlentities($frases->langtext));
